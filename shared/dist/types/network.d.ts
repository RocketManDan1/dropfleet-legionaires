import type { Vec2, MissionPhaseWire, FirePosture, MoveMode, MoraleState, ContactTier, DifficultyTier, MissionType } from './core.js';
...
export type GameEvent = ShotFiredEvent | ShotImpactEvent | UnitDestroyedEvent | SuppressionEvent | SmokeDeployedEvent;
...
export type ServerErrorCode = 'INVALID_ORDER' | 'UNIT_NOT_OWNED' | 'UNIT_DESTROYED' | 'UNIT_SUPPRESSED' | 'UNIT_ROUTING' | 'OUT_OF_RANGE' | 'NO_AMMO' | 'NO_LOS' | 'TARGET_NOT_ACQUIRED' | 'WEAPON_COOLDOWN' | 'QUEUE_FULL' | 'TRANSPORT_MOVING' | 'AUTH_FAILED' | 'SESSION_FULL' | 'MISSION_NOT_FOUND' | 'PHASE_WRONG' | 'NOT_IN_MISSION' | 'RATE_LIMITED' | 'INTERNAL_ERROR';
...
export type ServerMessage = {
    ...
    payload: MissionStateFullPayload;
} | {
    type: 'TICK_UPDATE';
    payload: TickUpdatePayload;
} | {
    type: 'ORDER_ACK';
    payload: OrderAckPayload;
} | {
    type: 'MISSION_PHASE';
    payload: MissionPhasePayload;
} | {
