...
const DIFFICULTY_FORCE_MULTIPLIER: Record<DifficultyTier, number> = {
  easy: 1.0,
  medium: 1.5,
  hard: 2.5,
};

const DIFFICULTY_PLATOON_COUNT: Record<DifficultyTier, number> = {
  easy: 2,
  medium: 3,
  hard: 5,
};
...
const MISSION_TIME_LIMITS: Record<DifficultyTier, number> = {
  easy: 1200,   // 20 min
  medium: 1500, // 25 min
  hard: 1800,   // 30 min
};
...
export interface MissionGenResult {
  ...
  enemyPlatoons: PlatoonState[];
  deploymentZoneCenter: Vec2;
  enemySpawnCenter: Vec2;
  timeLimitSec: number;
  /** Map from destroy objective IDs to their target unit IDs. */
  destroyTargets: Map<string, string[]>;
}
...
function findDryPos(terrain: TerrainData, centerX: number, centerZ: number, spreadRadius: number): Vec2 {
...
function createAmmoForType(unitType: UnitType): [AmmoState, AmmoState, AmmoState, AmmoState] {
...
// ---------------------------------------------------------------------------
// Per-mission-type objective patterns
// ---------------------------------------------------------------------------
...
interface ObjectivePattern {
  objectives: Array<{
    ...
    offsetFromEnemy: boolean; // true = near enemy; false = near player
    radius: number;
  }>;
}

const MISSION_PATTERNS: Partial<Record<MissionType, ObjectivePattern>> = {
  defend: {
    objectives: [
...
export function generateMission(
  ...
  const combatTypes = enemyTypes.filter(t =>
    t.unitClass === 'mbt' || t.unitClass === 'ifv' || t.unitClass === 'infantry' ||
  ...
  const enemyPlatoons: PlatoonState[] = [];
  ...
  for (let p = 0; p < platoonCount; p++) {
    ...
    let commandUnitId: string | null = null;
  ...
  const destroyTargets = new Map<string, string[]>();
  ...
  for (let i = 0; i < pattern.objectives.length; i++) {
    ...
    const pos = findDryPos(
      terrain,
      mapMidX + xBias,
      baseZ,
      Math.max(24, Math.min(mapW, mapH) * 0.08),
    );
    ...
    if (def.type === 'destroy') {
      // def.radius is in metres; positions are in cells — convert for comparison
      const searchRadiusCells = (def.radius * 2) / CELL_REAL_M;
      const targetIds = enemyUnits
        .filter(u => {
          const dx = u.posX - pos.x;
          const dz = u.posZ - pos.z;
          return dx * dx + dz * dz < searchRadiusCells * searchRadiusCells;
        })
        .map(u => u.instanceId);
...
function createEnemyUnit(
  ...
  return {
    ...
    maxEngageRangeM: Math.max(
      ...unitType.weapons.filter(Boolean).map(w => w!.rangeM),
      1000,
    ),
