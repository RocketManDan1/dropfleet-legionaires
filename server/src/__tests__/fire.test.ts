...
function buildRegistry(rangeM: number): UnitRegistry {
...
function buildScene(firerPosX: number, targetPosX: number): {
  ...
  const units = new Map<string, UnitInstance>([['firer', firer], ['target', target]]);
  const playerContacts = new Map<string, ContactEntry>();
