...
const STRIKE_CONFIGS: Record<StrikeType, StrikeConfig> = {
  artillery: {
...
export type StrikeType = 'artillery' | 'air_strike' | 'orbital';
...
interface StrikeConfig {
  ...
  roundCount: number;
  spreadM: number;
  aaInterceptable: boolean;
  baseSuppression: number;
}

/** An active (in-flight) strike request. */
export interface ActiveStrike {
  strikeId: string;
  type: StrikeType;
  requestingPlayerId: string;
  targetPos: Vec2;
  observerUnitId: string | null;
  /** Tick when the strike was requested. */
  requestedAtTick: number;
  /** Tick when the strike impacts. */
  impactTick: number;
  /** Whether the strike has been intercepted by AA. */
  intercepted: boolean;
  /** Whether the strike has already resolved (impacted). */
  resolved: boolean;
}

/** Result of a single strike impact on a unit. */
...
/** Result of processing a strike's impact. */
export interface StrikeImpactResult {
  strikeId: string;
  type: StrikeType;
  targetPos: Vec2;
  wasIntercepted: boolean;
  damageResults: StrikeDamageResult[];
}

/** Result of a strike request validation. */
...
/**
 * Dependency interface for the TheaterSupportManager.
 */
export interface TheaterSupportDependencies {
  /** Generate a unique ID. */
  generateId(): string;

  /** Check if an observer unit has LOS to a target position. */
  hasLOS(observerUnitId: string, targetPos: Vec2): boolean;

  /** Get a unit instance by ID. */
  getUnit(unitId: string): UnitInstance | null;

  /** Get all units within a radius of a position. */
  getUnitsInRadius(pos: Vec2, radiusM: number): UnitInstance[];

  /** Apply damage to a unit (delegates to combat system). */
  applyDamage(unitId: string, damage: number): boolean; // returns true if destroyed

  /** Apply suppression to a unit. */
  applySuppression(unitId: string, amount: number): void;

  /** Get the enemy faction for the current mission (for Khroshi EW check). */
  getEnemyFaction(): FactionId;
}
...
export class TheaterSupportManager {
  ...
  private activeStrikes: Map<string, ActiveStrike> = new Map();

  /** Remaining strike counts per type for this mission. */
  private remaining: Record<StrikeType, number> = {
    artillery: 3,     // TODO: Load from mission/difficulty config
    air_strike: 2,
    orbital: 1,
  };

  private deps: TheaterSupportDependencies;

  constructor(deps: TheaterSupportDependencies) {
    this.deps = deps;
  }
   ...
   */
  setAvailableStrikes(artillery: number, airStrikes: number, orbital: number): void {
   ...
   */
  getRemainingStrikes(): Record<StrikeType, number> {
   ...
   */
  tick(currentTick: number): StrikeImpactResult[] {
    const results: StrikeImpactResult[] = [];

    for (const [strikeId, strike] of this.activeStrikes) {
      if (strike.resolved) continue;
      if (currentTick < strike.impactTick) continue;
   ...
   */
  private resolveStrike(strike: ActiveStrike): StrikeImpactResult {
    ...
    // --- Apply blast damage ---
    const damageResults = this.applyBlastDamage(
      strike.targetPos,
      config,
    );
   ...
   */
  private rollAAInterception(targetPos: Vec2): boolean {
    ...
    const aaUnits = nearbyUnits.filter(unit => {
      ...
      const aaClasses: UnitClass[] = ['aa_vehicle', 'aa_infantry'];
  ...
  private applyBlastDamage(
    ...
    const results = new Map<string, StrikeDamageResult>();
    ...
    for (let round = 0; round < config.roundCount; round++) {
      ...
      for (const unit of nearbyUnits) {
        ...
        if (distance > config.blastRadiusM) continue;
   ...
   */
  getStrike(strikeId: string): ActiveStrike | null {
   ...
   */
  getTimeToImpactSec(strikeId: string, currentTick: number): number {
