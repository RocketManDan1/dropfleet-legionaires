 ...
 */
export async function loadUnitTypes(csvPath: string): Promise<UnitType[]> {
 ...
 */
export async function loadAllFactions(docsDir: string): Promise<UnitType[]> {
  const files = [
...
function parseUnitRow(cols: string[]): UnitType | null {
...
function parseWeaponSlot1(cols: string[]): WeaponSlot | null {
  const int = (i: number) => parseInt(cols[i] || '0', 10) || 0;
