...
// ---------------------------------------------------------------------------
// BT Node interface — simple protocol for tree traversal
// ---------------------------------------------------------------------------

interface BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult;
}
 ...
 */
class HasContactsInRange implements BTNode {
  private rangeM: number;

  constructor(rangeM: number) {
    this.rangeM = rangeM;
  }

  tick(ctx: PlatoonBTContext, _units: Map<string, UnitInstance>): BTResult {
 ...
 */
class PlatoonStrengthAbove implements BTNode {
 ...
 */
class CommandUnitAlive implements BTNode {
  tick(ctx: PlatoonBTContext, _units: Map<string, UnitInstance>): BTResult {
 ...
 */
class Inverter implements BTNode {
 ...
 */
class FlankRouteExists implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    ...
    if (len < 1) return { status: 'failure', orders: [] };
 ...
 */
class AdvanceToObjective implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    if (!ctx.nearestObjective) {
      return { status: 'failure', orders: [] };
    }

    const orders: PlatoonOrder[] = [];
 ...
 */
class MarchToObjective implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    if (!ctx.nearestObjective) {
      return { status: 'failure', orders: [] };
    }
 ...
 */
class HoldPosition implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    const orders: PlatoonOrder[] = [];
    for (const unitId of ctx.unitIds) {
      const unit = units.get(unitId);
 ...
 */
class SetFirePosture implements BTNode {
  private posture: 'free_fire' | 'return_fire' | 'hold_fire';

  constructor(posture: 'free_fire' | 'return_fire' | 'hold_fire') {
 ...
 */
class ConcentrateFireOnTarget implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    if (!ctx.nearestThreat) {
      return { status: 'failure', orders: [] };
    }
 ...
 */
class MoveAwayFromThreat implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    const centroid = computeCentroid(ctx, units);
 ...
 */
class FlankViaLowThreat implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    ...
    let bestFlankPos: Vec2 | null = null;
    ...
    const flankWaypoint: Vec2 = { x: waypointX, z: waypointZ };
 ...
 * the Synaptic Brood.
 */
class ScreenCommandUnit implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    if (!ctx.nearestThreat) {
      return { status: 'failure', orders: [] };
    }
 ...
 */
class ReverseToFallback implements BTNode {
  tick(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
    const centroid = computeCentroid(ctx, units);
    let fallbackPos: Vec2;
 ...
 */
function computeCentroid(ctx: PlatoonBTContext, units: Map<string, UnitInstance>): Vec2 {
  let sumX = 0;
  let sumZ = 0;
  let count = 0;
 ...
 */
function buildAtaxianAttackTree(): BTNode {
  return new Selector([
 ...
 */
function buildAtaxianDefendTree(): BTNode {
  return new Selector([
    // [Swarm Response]
    new Sequence([
      new HasContactsInRange(MEDIUM_ENGAGE_RANGE_M),
 ...
 *   [Default]        hold position + RETURN_FIRE
 */
function buildKhroshiDefendTree(): BTNode {
  return new Selector([
 ...
 */
function buildKhroshiAttackTree(): BTNode {
  return new Selector([
 ...
 */
function buildRetreatTree(): BTNode {
  return new Selector([
 ...
 */
function buildPatrolTree(): BTNode {
  return new Selector([
 ...
 */
function buildReinforceTree(): BTNode {
  return new Selector([
 ...
 */
function selectTree(faction: FactionId, intent: PlatoonIntent): BTNode {
  if (faction === 'ataxian') {
    switch (intent) {
...
export class PlatoonBehaviorTree {
   ...
   */
  private treeCache: Map<string, BTNode> = new Map();
   ...
   */
  tick(context: PlatoonBTContext, units: Map<string, UnitInstance>): BTResult {
  ...
  buildContext(
    ...
    for (const id of unitIds) {
      ...
      if (!u) continue;
      maxCrew += u.crewMax;
      if (!u.isDestroyed) currentCrew += u.crewCurrent;
    }
    ...
    let nearestObjective: Vec2 | null = null;
...
// Re-export the context builder import types for convenience
export type { PlatoonBTContext, BTResult, PlatoonOrder };
