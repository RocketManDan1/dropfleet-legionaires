import { BATLOC_PRESETS, type BatLocParams, resolveBatlocConfig } from './batloc.js';
...
export interface GenerateRequestError {
  ok: false;
...
export interface GenerateRequestSuccess {
  ok: true;
  width: number;
  height: number;
  seed?: number;
...
export type GenerateRequestResult = GenerateRequestSuccess | GenerateRequestError;
...
function parseMapSize(value: unknown, fallback: number, field: string): { ok: true; value: number } | { ok: false; error: GenerateRequestError } {
...
export function parseGenerateRequest(
  ...
  if (!widthResult.ok) return widthResult.error;

  const heightResult = parseMapSize(obj.height, fallbackHeight, 'height');
  if (!heightResult.ok) return heightResult.error;
  ...
  let batlocName: string | undefined;
  ...
  let overrides: Partial<BatLocParams> | undefined;
  if (obj.params !== undefined) {
    ...
    overrides = paramsObj as Partial<BatLocParams>;
  }
