...
import {
  INFLUENCE_SECURE_THRESHOLD,
  INFLUENCE_FALLEN_THRESHOLD,
...
export class PlanetStateManager {
   ...
   */
  getInfluenceState(planet: PlanetRecord): InfluenceState {
    ...
    if (
      planet.influenceAtaxian >= INFLUENCE_FALLEN_THRESHOLD ||
   ...
   */
  getControllingFaction(planet: PlanetRecord): FactionId | null {
    if (planet.influenceFederation > 50) return 'federation';
    if (planet.influenceAtaxian > 50) return 'ataxian';
    if (planet.influenceKhroshi > 50) return 'khroshi';
    return null;
  }
   ...
   */
  isPlanetLiberated(planet: PlanetRecord): boolean {
   ...
   */
  isPlanetFallen(planet: PlanetRecord): boolean {
    return (
      planet.influenceAtaxian >= INFLUENCE_FALLEN_THRESHOLD ||
      planet.influenceKhroshi >= INFLUENCE_FALLEN_THRESHOLD
    );
   ...
   */
  areMissionsAvailable(planet: PlanetRecord): boolean {
  ...
  setInfluence(
    ...
    if (Math.abs(sum - 100) > 0.01) {
      throw new Error(
        `Influence values must sum to 100, got ${sum} ` +
   ...
   */
  getAllInfluenceStates(planets: PlanetRecord[]): Map<string, InfluenceState> {
    const result = new Map<string, InfluenceState>();
   ...
   */
  getPlanetsInDanger(planets: PlanetRecord[]): PlanetRecord[] {
   ...
   */
  getFallenPlanets(planets: PlanetRecord[]): PlanetRecord[] {
   ...
   */
  private enforceInfluenceInvariant(planet: PlanetRecord): void {
