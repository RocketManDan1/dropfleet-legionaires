...
/** Summary of what happened during a single campaign tick, for logging. */
export interface CampaignTickReport {
  tickNumber: number;
  processedAt: number;
  planetsProcessed: number;
  influenceChanges: InfluenceChange[];
  transitsCompleted: TransitCompletion[];
  missionsExpired: string[];
  factionAIActions: FactionAIAction[];
}

/** A single influence change on a planet during this tick. */
...
/** A battalion that completed transit this tick. */
export interface TransitCompletion {
  battalionId: string;
  playerId: string;
  fromPlanetId: string;
  toPlanetId: string;
...
/** An action taken by the faction-level strategic AI. */
export interface FactionAIAction {
  faction: FactionId;
  actionType: 'expand' | 'reinforce' | 'attack' | 'consolidate';
  targetPlanetId: string;
  strength: number;
}

/**
 * Dependency interface — the CampaignTicker reads/writes data through
 * these callbacks so it stays decoupled from the persistence layer.
 */
export interface CampaignTickDependencies {
  /** Load all planets in the campaign. */
  loadAllPlanets(): Promise<PlanetRecord[]>;

  /** Save updated planet records. */
  savePlanets(planets: PlanetRecord[]): Promise<void>;

  /** Load all battalions currently in transit. */
  loadBattalionsInTransit(): Promise<BattalionRecord[]>;

  /** Update a battalion record (e.g. after transit completes). */
  saveBattalion(battalion: BattalionRecord): Promise<void>;

  /** Load all active (non-closed) mission records. */
  loadActiveMissions(): Promise<MissionRecord[]>;

  /** Mark a mission as expired. */
  expireMission(missionId: string): Promise<void>;

  /** Load/save the campaign tick state (current tick number, last tick time). */
  loadTickState(): Promise<CampaignTickState>;
  saveTickState(state: CampaignTickState): Promise<void>;
}
...
export class CampaignTicker {
  private deps: CampaignTickDependencies;
  ...
  constructor(deps: CampaignTickDependencies) {
    this.deps = deps;
  }
   ...
   */
  async processTick(): Promise<CampaignTickReport> {
    ...
    try {
      ...
      const report: CampaignTickReport = {
        tickNumber,
      ...
      const modifiedPlanets: PlanetRecord[] = [];
      ...
      // Update tick state
      const newTickState: CampaignTickState = {
        tickNumber,
   ...
   */
  private processInfluenceGrowth(
    planet: PlanetRecord,
    tickNumber: number,
  ): InfluenceChange[] {
    ...
    if (planet.influenceFederation >= 100) return changes;
    ...
    if (tickNumber % 2 === 1 && planet.influenceAtaxian > 0) {
      ...
      const growth = Math.min(
        growthRate,
        planet.influenceFederation, // can only eat Federation share
      );
   ...
   */
  private updateControllingFaction(planet: PlanetRecord): void {
   ...
   */
  private processTransits(transits: BattalionRecord[]): TransitCompletion[] {
    const completions: TransitCompletion[] = [];
    const now = Date.now();
   ...
   */
  private findStaleMissions(missions: MissionRecord[]): string[] {
    const now = Date.now();
    const stale: string[] = [];
  ...
  private runFactionAI(
    ...
    const planetMap = new Map<string, PlanetRecord>();
   ...
   */
  private runAtaxianAI(
    planets: PlanetRecord[],
    planetMap: Map<string, PlanetRecord>,
  ): FactionAIAction[] {
    const actions: FactionAIAction[] = [];

    for (const planet of planets) {
      // Skip planets with no Ataxian presence
      if (planet.influenceAtaxian <= 0) continue;
  ...
  private runKhroshiAI(
    ...
    for (const planet of planets) {
      if (planet.influenceKhroshi <= 0) continue;
   ...
   */
  private async catchUpMissedTicks(): Promise<void> {
    const tickState = await this.deps.loadTickState();
    const now = Date.now();
