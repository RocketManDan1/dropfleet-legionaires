...
/** Result of a battalion creation attempt. */
export interface CreateBattalionResult {
  success: boolean;
  battalion?: BattalionRecord;
  error?: string;
}

/** Result of a transit order. */
export interface TransitOrderResult {
  success: boolean;
  departsAt?: number;
  arrivesAt?: number;
  error?: string;
...
/** Result of modifying the OOB (unit slots). */
export interface OOBModifyResult {
  success: boolean;
  error?: string;
}

/**
 * Dependency interface — persistence callbacks so the BattalionManager
 * stays decoupled from the database.
 */
export interface BattalionDependencies {
  /** Load a battalion by ID. */
  loadBattalion(battalionId: string): Promise<BattalionRecord | null>;

  /** Save (insert or update) a battalion record. */
  saveBattalion(battalion: BattalionRecord): Promise<void>;

  /** Load all battalions owned by a player. */
  loadPlayerBattalions(playerId: string): Promise<BattalionRecord[]>;

  /** Load a player account (to check battalion count limits). */
  loadPlayer(playerId: string): Promise<PlayerAccount | null>;

  /** Update the player's battalion ID list. */
  updatePlayerBattalionIds(playerId: string, battalionIds: string[]): Promise<void>;

  /** Generate a unique ID. */
  generateId(): string;

  /** Look up a planet to verify it exists and is connected. */
  isPlanetConnected(fromPlanetId: string, toPlanetId: string): Promise<boolean>;
}
 ...
 */
function getDefaultTOE(type: BattalionType, generateId: () => string): UnitSlot[] {
  ...
  const makeSlot = (unitTypeId: string, crewMax: number, isReserve = false): UnitSlot => ({
    slotId: generateId(),
    unitTypeId,
    crewCurrent: crewMax,
    crewMax,
...
export class BattalionManager {
  private deps: BattalionDependencies;

  constructor(deps: BattalionDependencies) {
    this.deps = deps;
  }
  ...
  async createBattalion(
    ...
    const battalion: BattalionRecord = {
      battalionId,
      playerId,
      name: trimmedName,
      type,
  ...
  async orderTransit(
    ...
    const connected = await this.deps.isPlanetConnected(
      battalion.currentPlanetId,
      targetPlanetId,
    );
    if (!connected) {
      return {
   ...
   */
  async setReserveFlag(
    battalionId: string,
    slotId: string,
    isReserve: boolean,
  ): Promise<OOBModifyResult> {
    const battalion = await this.deps.loadBattalion(battalionId);
    if (!battalion) {
      return { success: false, error: 'Battalion not found' };
    }
    ...
    if (isReserve) {
      const activeAfter = battalion.unitSlots.filter(
        s => s.slotId !== slotId && !s.isReserve && s.status === 'active'
      ).length;
   ...
   */
  async getPlayerBattalions(playerId: string): Promise<BattalionRecord[]> {
   ...
   */
  async getBattalion(battalionId: string): Promise<BattalionRecord | null> {
   ...
   */
  async getDeployableSlots(battalionId: string): Promise<UnitSlot[]> {
    const battalion = await this.deps.loadBattalion(battalionId);
    if (!battalion) return [];

    return battalion.unitSlots.filter(
      slot => !slot.isReserve &&
   ...
   */
  async getReserveSlots(battalionId: string): Promise<UnitSlot[]> {
    const battalion = await this.deps.loadBattalion(battalionId);
    if (!battalion) return [];
