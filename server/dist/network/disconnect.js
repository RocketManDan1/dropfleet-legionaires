...
// ============================================================================
import { DISCONNECT_GRACE_TICKS, TICKS_PER_SEC, } from '@legionaires/shared/constants';
// ---------------------------------------------------------------------------
// DisconnectManager
// ---------------------------------------------------------------------------
/**
...
export class DisconnectManager {
     ...
     */
    onPlayerDisconnect(playerId, missionId, currentTick, playerUnitIds) {
        // If already tracking this player (double-disconnect), ignore
        if (this.graceEntries.has(playerId))
            return;
        const entry = {
            playerId,
            missionId,
     ...
     */
    tick(currentTick, missionPlayerCounts) {
        const results = [];
        for (const [playerId, entry] of this.graceEntries) {
            ...
            if (currentTick >= entry.expiresAtTick) {
                ...
                const allDisconnected = this.areAllPlayersDisconnected(entry.missionId, missionPlayerCounts);
                results.push({
                    playerId,
                ...
                if (allDisconnected) {
                    this.callbacks.triggerMissionDefeat(entry.missionId, 'All players disconnected — mission lost');
                }
                this.graceEntries.delete(playerId);
            }
        }
     ...
     */
    getRemainingGraceSec(playerId, currentTick) {
        const entry = this.graceEntries.get(playerId);
        if (!entry)
            return 0;
     ...
     */
    getDisconnectedPlayers(missionId) {
        const result = [];
        for (const [playerId, entry] of this.graceEntries) {
     ...
     */
    areAllPlayersDisconnected(missionId, missionPlayerCounts) {
