 ...
 */
export async function loadWeaponTable(csvPath) {
    const raw = await readFile(csvPath, 'utf-8');
    const lines = raw.split('\n').filter(l => l.trim().length > 0);
    const table = new Map();
    for (let i = 1; i < lines.length; i++) {
        const c = parseCSVLine(lines[i]);
        const int = (idx) => parseInt(c[idx] || '0', 10) || 0;
 ...
 */
export async function loadUnitTypes(csvPath, weaponTable) {
    const raw = await readFile(csvPath, 'utf-8');
    const lines = raw.split('\n').filter(l => l.trim().length > 0);
    if (lines.length < 2)
        return [];
 ...
 */
export async function loadAllFactions(docsDir, weaponsDir) {
...
function parseUnitRow(cols, weaponTable) {
...
function parseWeaponSlot1(cols, weaponTable) {
    const int = (i) => parseInt(cols[i] || '0', 10) || 0;
...
function parseWeaponSlot234(cols, idCol, nameCol, heCol, apCol, soundCol, weaponTable) {
    const int = (i) => parseInt(cols[i] || '0', 10) || 0;
    const id = int(idCol);
    if (id === 0)
        return null;
