...
// ============================================================================
import { MOVE_MULT_MARCH, MOVE_MULT_ADVANCE, MOVE_MULT_REVERSE, } from '@legionaires/shared';
// ---------------------------------------------------------------------------
 ...
 */
function classifySpeedState(recentDistanceM, stoppedForSec) {
    if (recentDistanceM > SPEED_THRESHOLD_FAST)
        return 'fast';
    if (recentDistanceM > SPEED_THRESHOLD_SLOW)
        return 'slow';
    if (stoppedForSec >= FULL_HALT_THRESHOLD_SEC)
        return 'full_halt';
    return 'short_halt';
}
// ---------------------------------------------------------------------------
 ...
 */
function effectiveSpeed(maxSpeedM, moveMode, cellCost) {
 ...
 */
function getTerrainCost(costGrids, moveClass, x, z) {
    if (!costGrids)
        return 1.0;
 ...
 */
export function resolveMovement(units, dt, terrain, costGrids, unitTypes, currentTick) {
    const tick = currentTick ?? 0;
    for (const [unitId, unit] of units) {
        // Skip dead, surrendered, or frozen units
        if (unit.isDestroyed)
            continue;
        if (unit.moraleState === 'surrendered')
            continue;
        // Skip frozen (disconnected player) units — indicated by a freeze timestamp
        if (unit.frozenAtTick != null)
            continue;
