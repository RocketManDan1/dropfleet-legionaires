 ...
 */
export function applyDamage(shotRecords, units, tick, unitTypes) {
    const damageResults = [];
...
// ── Helpers ─────────────────────────────────────────────────────────────────
function getWeapon(unitTypes, unitTypeId, slot) {
    if (!unitTypes)
        return null;
...
function determineImpactFacing(shot, target) {
    ...
    if (hitTurret) {
        return `turret${turretZone.charAt(0).toUpperCase() + turretZone.slice(1)}`;
    }
    return `hull${hullZone.charAt(0).toUpperCase() + hullZone.slice(1)}`;
}
function rollPenetration(shot, target, facing, unitTypes) {
...
function rollCrewDamage(shot, target, penResult, unitTypes) {
    const targetType = getUnitType(unitTypes, target.unitTypeId);
...
function rollSystemDamage(_shot, _target, penResult) {
...
export function enrichShotWithPen(shot, units, unitTypes) {
    const firer = units.get(shot.firerId);
    if (!firer || !unitTypes)
        return { penValue: 10, wh: 0 };
    const weapon = getWeapon(unitTypes, firer.unitTypeId, shot.weaponSlot);
    if (!weapon)
        return { penValue: 10, wh: 0 };
    let penValue;
    switch (shot.ammoType) {
        case 'AP':
            penValue = weapon.penAP;
            break;
        case 'Sabot':
            penValue = weapon.penSabot;
            break;
        case 'HEAT':
            penValue = weapon.penHEAT;
            break;
        case 'HE':
            penValue = weapon.heKill;
            break;
