...
// Milestones 2 + 3
// ============================================================================
import { TICK_RATE_HZ, TICK_MS, TICKS_PER_SEC, AI_STRATEGIC_UPDATE_TICKS, AI_PLATOON_BT_TICKS, MAX_QUEUED_WAYPOINTS, } from '@legionaires/shared';
...
export class TickLoop {
    ...
    executeTick() {
        ...
        if (this.lastTimings.totalMs > TICK_MS) {
            console.warn(`[TickLoop] Tick ${this.tick - 1} exceeded budget: ` +
            ...
            for (const orderMsg of pendingOrders) {
                ...
                if (orderMsg.type === 'move' || orderMsg.type === 'move_fast' || orderMsg.type === 'reverse') {
                    if (orderMsg.targetPos) {
                        const moveMode = orderMsg.type === 'move_fast'
                            ? 'march'
    ...
    phaseCommandPropagation() {
        ...
        if (strategicAI && influenceMapMgr && this.tick % AI_STRATEGIC_UPDATE_TICKS === 0) {
            ...
            const decision = strategicAI.update(this.tick, units, platoons, objectives, influenceMaps);
        ...
        if (platoonBT && this.tick % AI_PLATOON_BT_TICKS === 0) {
            ...
            if (cachedMaps) {
                for (const [, platoon] of platoons) {
                    ...
                    if (platoon.factionId === 'federation')
                        continue;
                    ...
                    for (const uid of platoon.unitIds) {
                        ...
                        for (const [, otherUnit] of units) {
                            if (otherUnit.isDestroyed || otherUnit.ownerId === u.ownerId)
                                continue;
                    ...
                    const ctx = platoonBT.buildContext(platoon.platoonId, platoon.factionId, platoon.intent, platoon.unitIds, platoon.commandUnitId, units, cachedMaps, objectives, nearestEnemy);
     ...
     */
    normalizeAIOrder(orderType, targetPos, targetUnitId) {
    ...
    phaseFireResolution(dt) {
        ...
        for (const [id, unit] of units) {
            if (unit.isDestroyed)
                continue;
            if ((unit.currentOrder?.type === 'engage' ||
     ...
     */
    phaseSuppressionMorale(dt, isSecondTick) {
        const units = this.session.getUnitRegistry();
    ...
    phaseMissionLifecycle() {
        ...
        for (const [, conn] of players) {
            if (conn.isConnected) {
                playerCount++;
                if (!conn.readyForDeployment)
                    allReady = false;
                if (!conn.acknowledgedAAR)
                    allAckedAAR = false;
            }
        }
