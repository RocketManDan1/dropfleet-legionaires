...
import { TICKS_PER_SEC, DEPLOYMENT_TIMER_SEC, } from '@legionaires/shared';
...
import { filterContactsForPlayer, filterUnitsForPlayer, parseClientMessage, serializeServerMessage, } from './network/protocol.js';
...
let terrain = generateTerrain(512, 512);
...
const allUnitTypes = await loadAllFactions(path.join(repoRoot, 'New Direction Docs'), path.join(repoRoot, 'Unit Testing'));
unitTypeRegistry.load(allUnitTypes);
console.log(`Loaded ${unitTypeRegistry.size} unit types`);
// ---------------------------------------------------------------------------
// M3: Mission configuration
// ---------------------------------------------------------------------------
const MISSION_TYPE = 'defend';
const MISSION_DIFFICULTY = 'easy';
const ENEMY_FACTION = 'ataxian';
// ---------------------------------------------------------------------------
// Create session with M3 lifecycle
// ---------------------------------------------------------------------------
const session = new GameSession(`m3-mission-${Date.now().toString(36)}`, terrain, {
...
const missionGen = generateMission(MISSION_TYPE, MISSION_DIFFICULTY, ENEMY_FACTION, terrain, unitTypeRegistry, 0);
...
deploymentMgr.generateZone(terrain.width, terrain.height, 1, MISSION_TYPE, missionGen.deploymentZoneCenter ? { x: missionGen.deploymentZoneCenter.x, z: missionGen.deploymentZoneCenter.z } : undefined, terrain.terrainTypeMap, terrain.heightmap, terrain.seaLevel);
...
const strategicAI = new StrategicAI(ENEMY_FACTION);
...
let unitOrdinal = 0;
function createAmmoForUnit(unitTypeId) {
...
function createPlayerUnit(playerId, unitTypeId, x, z) {
    ...
    return {
        ...
        maxEngageRangeM: Math.max(...template.weapons.filter(Boolean).map(w => w.rangeM), 1000),
...
function buildMissionSnapshotFor(playerId) {
    const units = filterUnitsForPlayer(playerId, session.getUnitRegistry(), session.getContactMap().get(playerId) ?? new Map(), unitTypeRegistry);
    ...
    const wirePhase = (phaseMap[session.getPhase()] ?? 'briefing');
    return {
        ...
        units,
        contacts,
        objectives: session.getObjectives().map((o) => ({
...
function mapMoveOrderToMode(orderType, moveMode) {
    if (moveMode)
        return moveMode;
    if (orderType === 'move_fast')
        return 'march';
...
wss.on('connection', (ws) => {
    ...
    ws.send(serializeServerMessage({ type: 'MISSION_STATE_FULL', payload: buildMissionSnapshotFor(playerId) }, session.getTickLoop().getCurrentTick()));
    // M3: Send deployment zone if in deployment phase
    const phase = session.getPhase();
    if (phase === 'deployment' || phase === 'created') {
        ws.send(serializeServerMessage({ type: 'DEPLOYMENT_ZONE', payload: buildDeploymentZone() }, session.getTickLoop().getCurrentTick()));
    }
    ...
    ws.send(serializeServerMessage({
        type: 'MISSION_PHASE',
        payload: {
            phase: (wirePhaseMap[phase] ?? 'briefing'),
            missionTimeSec: session.getTickLoop().getCurrentTick() / TICKS_PER_SEC,
        },
    ...
    ws.on('message', (raw) => {
        ...
        try {
            ...
            if (msg.type === 'UNDEPLOY_UNIT') {
                ...
                const sendUndeployResult = (success, reason) => {
                    ws.send(serializeServerMessage({
                        type: 'UNDEPLOY_UNIT_RESULT',
            ...
            if (parsed.type === 'ORDER') {
                ...
                if (rejectReason) {
                    ws.send(serializeServerMessage({ type: 'ORDER_ACK', payload: { orderId, unitId, status: 'REJECTED', reason: rejectReason } }, session.getTickLoop().getCurrentTick()));
                }
                else {
                    const order = {
                        unitId,
                    ...
                    ws.send(serializeServerMessage({ type: 'ORDER_ACK', payload: { orderId, unitId, status: 'ACCEPTED' } }, session.getTickLoop().getCurrentTick()));
                }
            }
            ...
            if (parsed.type === 'DEPLOY_UNIT') {
                const dm = session.getDeploymentManager();
                const sendDeployResult = (unitId, success, reason, posX, posZ) => {
                    ws.send(serializeServerMessage({
                        type: 'DEPLOY_UNIT_RESULT',
                        payload: { unitId, success, reason, posX, posZ },
                    }, session.getTickLoop().getCurrentTick()));
                };
                const { unitId, posX, posZ } = parsed.payload;
                if (!dm) {
