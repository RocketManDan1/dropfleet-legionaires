...
const session = new GameSession('m2-skirmish-sandbox', terrain, {
...
function createSandboxUnit(playerId, x, z) {
...
function buildMissionSnapshotFor(playerId) {
    const units = filterUnitsForPlayer(playerId, session.getUnitRegistry(), session.getContactMap().get(playerId) ?? new Map());
