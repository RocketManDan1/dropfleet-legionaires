 ...
 */
function computePlatoonCentroid(platoon, units) {
    let sumX = 0;
    let sumZ = 0;
    let count = 0;
 ...
 */
function computePlatoonStrength(platoon, units) {
    let currentCrew = 0;
    let maxCrew = 0;
    for (const unitId of platoon.unitIds) {
        const unit = units.get(unitId);
 ...
 * is still alive.
 */
function isCommandUnitAlive(platoon, units) {
    if (!platoon.commandUnitId)
        return false;
 ...
 */
function getWeights(faction) {
    if (faction === 'ataxian')
        return ATAXIAN_FULL_WEIGHTS;
...
export class StrategicAI {
     ...
     */
    update(tick, units, platoons, objectives, influenceMaps) {
        const assignedIntents = new Map();
        ...
        for (const platoon of aiPlatoons) {
            ...
            for (const objective of activeObjectives) {
                const objPos = { x: objective.posX, z: objective.posZ };
        ...
        const objectiveAttackCount = new Map();
        const objectiveDefendCount = new Map();
        const assignedPlatoons = new Set();
        for (const entry of scores) {
            // Skip platoons that already got an intent (retreat or prior assignment)
            if (assignedIntents.has(entry.platoonId))
                continue;
            if (assignedPlatoons.has(entry.platoonId))
                continue;
        ...
        // -----------------------------------------------------------------
        const reinforcementTarget = this.selectReinforcementTarget(activeObjectives, influenceMaps);
        // -----------------------------------------------------------------
        ...
        // -----------------------------------------------------------------
        const retreatingPlatoons = aiPlatoons.filter((p) => assignedIntents.get(p.platoonId) === 'retreat');
        let retreatRoute = null;
        if (retreatingPlatoons.length > 0) {
            ...
            retreatRoute = this.buildRetreatRoute(retreatingPlatoons, units, influenceMaps);
        }
        // -----------------------------------------------------------------
        // Cache and return
        // -----------------------------------------------------------------
     ...
     * active objectives.
     */
    selectReinforcementTarget(objectives, influenceMaps) {
        if (objectives.length === 0)
            return null;
        let lowestControl = Infinity;
        let target = null;
        for (const obj of objectives) {
            const pos = { x: obj.posX, z: obj.posZ };
     ...
     */
    buildRetreatRoute(retreatingPlatoons, units, influenceMaps) {
        ...
        if (count === 0)
            return [];
        const startPos = { x: totalX / count, z: totalZ / count };
